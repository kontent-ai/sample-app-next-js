...
import { HeroImage } from "../../../../components/landingPage/ui/heroImage.tsx";
import { PersonHorizontal } from "../../../../components/shared/PersonHorizontal.tsx";
import { RichTextElement } from "../../../../components/shared/richText/RichTextElement.tsx";
...
import { formatDate } from "../../../../lib/utils/dateTime.ts";
...
const ArticlePage = async ({ params }: { params: Promise<{ envId: string; slug: string }> }) => {
  ...
  const articleData = await getArticleBySlug(
  ...
  return (
    <AppPage item={article}>
      <HeroImage url={article.elements.hero_image.value[0]?.url ?? ""} itemId={article.system.id}>
        <div className="py-1 px-3 w-full md:w-fit bg-mainBackgroundColor opacity-90">
          <h1 className="m-0 text-white  text-5xl tracking-wide font-semibold">
            {article.elements.title.value}
          </h1>
        </div>
        <div className="p-4">
          <p className="font-semibold text-white text-justify">{article.elements.abstract.value}</p>
        </div>
      </HeroImage>
      <div className="px-2 max-w-screen m-auto md:px-20">
        {article.elements.author.linkedItems[0] ? (
          <PersonHorizontal item={article.elements.author.linkedItems[0]} />
        ) : null}
        <div className="flex flex-col gap-2">
          <div className="w-fit p-2 bg-gray-800 text-white opacity-90 font-semibold">
            {article.elements.publishing_date.value
              ? formatDate(article.elements.publishing_date.value)
              : null}
          </div>
        ...
        <RichTextElement element={article.elements.content} isInsideTable={false} />
      </div>
    </AppPage>
  );
};
