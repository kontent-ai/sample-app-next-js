      ...
      <div className="max-w-7xl mt-20 mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row">
        <figure className="lg:flex-1 rounded-lg flex items-center justify-center not-prose">
          {product.elements.product_base__main_image.value[0] ? (
            <Image
              ...
              alt={
                product.elements.product_base__main_image.value[0].description ??
                product.elements.product_base__main_image.value[0].url.split("/").pop() ??
                "Product image"
              }
              width={widthLimit}
        ...
        <div className="lg:flex-2 px-4 flex flex-col gap-0">
          ...
          <p
            ...
            {...createElementSmartLink(
              contentTypes.product.elements.product_base__description.codename,
            )}
          >
...
export const generateStaticParams = async () =>
  getProductItemsWithSlugs({ envId: defaultEnvId }).then((products) =>
    products.map((product) => ({ slug: product.elements.slug.value })),
  );
