...
import { getItemByCodename } from "../../../lib/kontentClient.ts";
...
export const GET = async (req: NextRequest) => {
  ...
  if (typeof productCodename !== "string") {
    return new Response(
      "You have to provide 'codename' query parameter with the product's codename.",
      { status: 400 },
    );
  }
  ...
  const product = await getItemByCodename<Product>(
