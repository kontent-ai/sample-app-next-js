...
import { StandaloneSmartLinkButton } from "../shared/StandaloneSmartLinkButton.tsx";
...
export const ProductItem: FC<Props> = (props) => (
  <li className="min-w-full m-0 p-0 relative rounded-lg shadow hover:shadow-xl transition-shadow border border-gray-200 cursor-pointer min-h-full">
    <Link href={props.detailUrl} className="group no-underline p-0 m-0">
      <StandaloneSmartLinkButton itemId={props.itemId} />

      <div className="flex flex-col gap-2">
        <h5 className="px-4 pt-2 mt-2 text-center text-xl  tracking-wider font-semibold text-gray-900 line-clamp-1">
          {props.title}
        </h5>
        ...
        {props.price ? (
          <p className="m-0 text-center text-xl prose font-normal pb-2">{`${props.price}€`}</p>
        ) : null}
        <button
          type="button"
          className="group-hover:bg-mainColorHover bg-mainButtonColor text-white block ml-auto w-fit mb-3 mr-4 font-semibold line-clamp-6 hover:bg-mainHoverColor border py-2 px-4 rounded"
        >
          Detail
        </button>
      </div>
