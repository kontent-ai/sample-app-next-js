...
import { ProductsPageSize } from "../../lib/constants/paging.ts";
import { resolveUrlPath } from "../../lib/routing.ts";
import { updateSearchParams } from "../../lib/utils/searchParamsUtils.ts";
import type { Product } from "../../models/index.ts";
import { ProductItem } from "../listingPage/ProductItem.tsx";
...
const ProductsListing: FC = () => {
  ...
  const pageNumber = useMemo(() => (!page || Number.isNaN(+page) ? 1 : +page), [page]);
  ...
  const getProductCategories = useCallback(async () => {
    ...
    const productCategories = (await response.json()) as ITaxonomyTerms[];
  ...
  const renderFilterOption = (term: ITaxonomyTerms) => {
    const onCheckBoxClicked = (isChecked: boolean) => {
      const newCategories = isChecked
        ...
        : categories.filter(
            (c) => c !== term.codename && !term.terms.map((t) => t.codename).includes(c),
          );
    ...
    return (
      <li key={term.codename} className="m-0 p-0">
        ...
        {term.terms.length > 0 && (
          <ul className="list-none">{term.terms.map(renderFilterOption)}</ul>
        )}
      </li>
    );
  };
