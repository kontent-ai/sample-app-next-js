import { type IContentItem, type Elements } from '@kontent-ai/delivery-sdk';
/**
 * Generated by '@kontent-ai/model-generator@5.10.0'
 *
<<<<<<<< HEAD:models/content-types/Block_HeroUnit.ts
 * 🧱 Hero unit
========
 * UMLP component – Hero unit
>>>>>>>> 6147910 (make responsive, add smartlink, regenerate models):models/content-types/UMLPComponent_HeroUnit.ts
 * Id: 76f5b7c0-3d2d-40d9-a5a9-3e4d8ec71a54
 * Codename: hero_unit
 */
<<<<<<<< HEAD:models/content-types/Block_HeroUnit.ts
export type Block_HeroUnit = IContentItem<{
========
export type UMLPComponent_HeroUnit = IContentItem<{
>>>>>>>> 6147910 (make responsive, add smartlink, regenerate models):models/content-types/UMLPComponent_HeroUnit.ts
  /**
   * Background image (asset)
   * Required: false
   * Id: f00f8f79-3a3b-4098-8143-c52f0804f27b
   * Codename: background_image
   */
  backgroundImage: Elements.AssetsElement;

  /**
   * Subtitle (text)
   * Required: false
   * Id: a63b8dfd-3f13-43b7-9a63-02bf070c3cb0
   * Codename: subtitle
   */
  subtitle: Elements.TextElement;

  /**
   * Title (text)
   * Required: true
   * Id: 250e0af2-9305-47b8-acdb-9b65c70cbc06
   * Codename: title
   */
  title: Elements.TextElement;
}>;
